...
class RegisterController extends GetxController {
  ...
  void login1() {
    if (loginFormKey1.currentState!.validate()) {
      checkUser1( usernameController1.text,phonenumberController1.text,emailController1.text, passwordController1.text,).then((auth) {
        if (auth) {
          Get.snackbar('Sign', 'Sign  successfully');
        } else {
          Get.snackbar('Sign', 'Invalid email or password or phonenumber or username');
        }
...
Future<bool> checkUser1(String user, String password,String username,String phonenumber) {
    if ( username == 'abc'&& phonenumber == '123'&&user == 'foo@foo.com' && password == '123') {
