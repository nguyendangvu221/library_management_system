...
class CreateOrEditUserScreen extends GetView<UserController> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(
          isUpdate ? "Sửa hồ sơ" : "Thêm tài khoản",
              ...
              child: Center(
                child: CustomTextButton(
                  ...
                  onTap: controller.validate()
                      ? () async {
                          if (isUpdate) {
                            String message = await controller.updateProfile();
                            if (context.mounted) {
                              message == '200'
                          ...
                          if (context.mounted) {
                            controller.isSave.value
                                ...
                                : ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content:
                                          isUpdate ? const Text('cập nhật thất bại') : const Text('Khởi tạo thất bại!'),
                                    ),
                                  );
                          }
                      ...
                      : null,
                  textColor: controller.validate() ? AppColor.black : AppColor.grey.shade500,
                ),
              ),
            ),
          ),
